...
Пока мы еще залогированы под пользователем ubuntu создадим нового пользователя и его пароль:

```bash
sudo useradd [user] -m -G -G 4,27,1000 -c "[Имя-Фамимлия-прочее-прочее-для-информации"
sudo passwd [user]
```
...
И напоследок, убираем `#` в строчке `#Banner /etc/issue.net`. Баннеры -- это красиво и пксть он приветсвует нас. Сохраняем файл `/etc/ssh/sshd_config` -- `Ctrl+O` и `Enter`, а выходим из редактора `Ctrl+X`. Создаем наш красивый баннер:
...
```
                       |  Raspbian GNU/Linux 8.0 (jessie)  _)
    __| _` |  __| __ \  __ \   _ \  __|  __| |   |    __ \  |
   |   (   |\__ \ |   | |   |  __/ |    |    |   |    |   | |
  _|  \__,_|____/ .__/ _.__/ \___|_|   _|   \__, |    .__/ _|
                 _|                         ____/    _|
```
Можете найти в интернте онлайн-утилиты и сделать что-то своё.  Сохраняем баннер `Ctrl+O` и `Enter`, а выходим из редактора `Ctrl+X`.
 
 

Красиво раскрашиваем командную строку:
```
export PS1='\[\e[1;32m\] \u\[\e[1;33m\]@\[\e[1;32m\]\h\[\e[0;36m\] [\W]\[\e[1;32m\]: \[\e[0;37m\] '
```


Первым делом нам понадобится настроить пользователя, от имени которого мы будем работать и SSH-доступ для него.
...
Во-вторых, нам понадобится сервер базы данных МySQL. Установим его:
