...
**!!!КСТАТИ, ВАЖНО: ВСЕ ЗНАЧЕНИЯ ПОМЕЩЕННЫЕ В КВАДРАТНЫЕ СКОБКИ НАДО ЗАМЕНИТЬ НА СВОИ СОБСТВЕННЫЕ. ЭТО КАСАЕТСЯ ИМЕНИ [user], ПАРОЛЕЙ [password] И ВСЕГО ТАКОГО ПРОЧЕНО. В ОБЩЕМ, ВЫ МЕНЯ ПОНЯЛИ!!!** 
...
Теперь можно обновить репозиторий. Это особенно важно если разворачиваем на новой машинке или свежей VPS, где он полностью пустой:
```bash
sudo apt-get update
```
